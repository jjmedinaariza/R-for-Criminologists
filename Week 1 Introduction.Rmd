...
[Section 1.2: Vectors, factors and data frames](#vectors-factors-and-data-frames).
[Section 1.3: Data manipulation](#data-manipulation-subsetting-and-selection-of-rows-and-columns).
[Section 1.4: Loading data and packages](#loading-data-and-packages).
[Section 1.5: A first look at the data](#a-first-look-at-the-data).
[Section 1.6: Numerical summaries of central tendency and variability](#numerical-summaries-of-central-tendency-and-variability).
[Section 1.7: Summarising categorical variables](#summarising-categorical-variables).
[Section 1.8: Saving your work](#saving-your-work).

#1.1 Introduction

This course provides an introduction to data analysis using R, a programming language and environment -or as some argue the [lingua franca of statistics](http://vimeo.com/52999628). You can learn more about the history of R clicking [here](https://www.youtube.com/watch?v=kzxHxFHW6hs&list=PLjTlxb-wKvXNSDfcKPFH2gzHGyjpeCZmJ). 

This handout contains various hyperlinks to videos that explain or expand some of the ideas and concepts introduced. It will be handy if you have speakers or headphones to listen to the videos.

In order to follow this handout at home you would need to install both R and RStudio in your local machine -click [here](https://www.youtube.com/watch?v=eD07NznguA4) for instructions using Windows or [here](https://www.youtube.com/watch?v=cX532N_XLIs&list=PLqzoL9-eJTNDw71zWePXyHx3_cm_fMP8S) for instructions using a Mac. Alternatively, you could use a PC in one of the University computer clusters. 
...
+ It is important that you **moderate your expectations**. You would not expect to be the main act at Glastonbury after just a few guitar lessons. Bu even if you never sell a million records, you can get pretty good at guitar playing by practicing. Or going back to the language methaphor, you may not be able to write like Shakespeare, but as long as you can communicate with others in a functional way that's what really matters.

+ **Get used to making mistakes!** You will make loads of them and there's absolutely no shame on that. It's normal and an essential part of the learning process. We learnt by receiving good feedback whem making mistakes. My job is to give you that feedback and yours to ask me for help.

+ **Don't aim to memorise everything.** There are over 5000 **packages** in R (modules that expand the functionality of R). Each of them create additional **functions** that allow you to do new cool stuff with your data. Nobody expects you to know by heart all of these functions and what they do. Memorising them would be as useless as memorising a dictionary. Instead, as you use R more and more your memory of basic functions (those you use the most) will improve. But in the day of searchable help files, google, and resources such as [Quick-R](http://www.statmethods.net/) or the [R-Codebook](http://www.cookbook-r.com/) (providing quick "cheatsheets" for common commands), the need to fill your memory with these functions is considerably reduced. 

#1.2 Interacting with R and creating objects

Unlike other programs for data analysis you may have used in the past (Excel, SPSS), you need to interact with R by means of writing down instructions and asking R to evaluate those instructions. You can write in the **command line** directly, but most typically you will write in a **script**, a text file, from which you can also run commands and that has the advantage that can be saved. Later I will point you to a short 6-minute video that show you how to work with scripts.

In these handouts you will see bits of these instructions (I will often refer to this as "code") inside greyed boxes and the result of executing those instructions in white boxes. If you are looking at this document and have an open session of R, you should be able to reproduce the code by cutting it and pasting it into your R console (or script). 
...
Once you press enter, the R engine will evaluate this expression and do something with it. In this case, it will tell you that the result of adding 3 and 5 is 8.

Using R simply as a calculator would be akin to killing flies with nuclear missiles. Typically we want to do slightly more interesting things with it. We use R for analysing and visualising data. And data in R is stored as **objects**. R is what we call an *object-oriented language*. Everything that exists in R is an object. 

If you are used to other programs such as Excel or SPSS the first mental barrier you need to cross in order to understand R is that (unlike in those programs), data in R can come in many shapes. Excel or SPSS files come in a tabular spreadsheet form, with rows typically representing cases and columns representing characteristics or attributes of those cases (what we also call variables). Typically, you can only work with one of these data tables at the time. R can work with this sort of spreadsheet data structure. However, it can also work with data that comes in many other shapes and it can work with several of these objects simultaneously. This is part of  makes R so flexible and special.

We can create objetcs in R by asking R to put things inside of named objects. For that we use the **assingment operator**. In R the `<-` symbol is the assignment operator (you could also use the `=` sign as an assignment operator, *but don't*). This operator is what assigns value to a symbol. So, for example, if we type the following expression in the prompt:

```{r}
x <- 5
```

We are simply telling R to create a *numeric object*, called `x`, with one element (5) or of lenght 1. It is numeric because we are putting a number inside this object. It may help you at this stage to think of objects as boxes, things where you store stuff and the assignment operator as the tool you use to tell R what goes inside.

When a complete expression is entered at the prompt in the main consele and we press *return*, this expression is then evaluated by the R engine and the result is returned. This result may be auto-printed in the main console or not (and the latter may take some get used to if you've used before something like SPSS). If you press return after entering the previous expression you won't see much happening in the main console (although if you are using RStudio you may have noticed that a new object, X, appears in the global environement window in RStudio).
...
Or alternatively, you can use a **function**. R uses functions to perform operations. Everything you do in R is the result of running a function. You can think of functions as preprogrammed routines that ask R to do a particular something. Here we can use the `print()` function. 

```{r}
print(x)
```

When writing expressions in R is very important you understand that **R is case sensitive**. This can drive you nuts if you are not careful. More often than not if you write an expression asking R to do something and R return an error message, chances are that you have used lower case when upper case was needed (or viceversa). So always check for the right spelling. For example, see what happens if I ask the following:
...
In sum, everything that exist in R is an object and everything you do in R is the result of running a function. In fact, R does not have, as other programming languages. So, the numbers we use in the sum operation earlier are numeric objects and the sum operator is in fact a function for summation.
...
When you create objects you will place them in your working memory or workspace. Think of it as a warehouse where you are placing a bunch of boxes (your objects). R works using your RAM. That's where all these objects get stored, which means you need good RAM for very large datasets. In RStudio you can visualise the objects you have created during a session in the **Global Environment** screen. But if you want to produce a list of what's there you can use the `ls()` function (the results you get my differ from the ones below depending on what you actually have in your global environment).
...
```{r}
the_smiths <- factor(c("Morrisey", "Marr", "Rourke", "Joyce"))
the_smiths
#Alternatively for similar result using the as.factor() function
the_smiths_bis <- c("Morrisey", "Marr", "Rourke", "Joyce")
the_smiths_f <- as.factor(the_smiths_bis)
the_smiths_f
...
+ You don't want them to be too long (or you will regret it everytime you need to use that object in your analysis: your fingers will bleed from typing).
...
You may want to look at the [style guide](http://google-styleguide.googlecode.com/svn/trunk/Rguide.xml) used by Google programers using R for further details, but the basic conventions listed above will possibly suffice for now.
...
```{r}
Premiership13_14[Premiership13_14$teams == "Man City", ] #We are asking for rows where the condition is met, which is why this appears before the comma within the brackets, we are not selecting particular columns -and thus don't specify an additional argument after the comma.
```
...
The number 4 and 5 that you see indicate that those are the respective row numbers for those two cases. You will have noticed a comma within those square brackets, whatever you put after the comma deals with columns rather than rows. 
...
If we use ordinary filtering NA values will be included in our selection:
...
For other logical and arithmetic operators that you can use in R for filtering (and how to write them) in R please look [here](http://www.statmethods.net/management/operators.html). For further details you may also want to see [Roger Peng's video on subsetting](http://tinyurl.com/ot8cq96).
...
For the most part the data we use in the course have already being formatted and cleaned to make your life easier. But in real life data, even pre-processed data, tends to be messy. The day to day of data analysis implies spending the bulk of the time (up to 90%) in what some informally call **data munging** before you can start exploring your data for interesting patterns. Pre-processing data may involve: fixing variable names; creating new variables; merging data sets; reshaping data sets; dealing with missing data; transforming variables; checking on and removing inconsistent values; etc. Using R means that you will be induced to have a much better (and essential) record of all these operations (by saving the scripts performing them).
...
Another way of getting this sort of file would be using the following approach:
...
Once in your computer you can load the file into your working environment everytime you need it for a different project using the `load()` function.
...
One of the beauties of R is that anybody can expand its functionality. There are over 5000 **packages** containing collections of functions that expand the functionality of R. Watch [this tutorial](https://www.youtube.com/watch?v=3RWb5U3X-T8&index=11&list=PLqzoL9-eJTNDw71zWePXyHx3_cm_fMP8S) for how to install and load a package into R. In this course we use packages that are not part of the basic installation of R, which means you will have to install them in your local maching or in your p:drive if you are using a computer at any of the University computer clusters. You can see the packages that are installed locally in the packages tab in RStudio. Or you could use the `library()` function with no arguments.
...
The first of this is the `Boston` dataset from the `MASS` package that is part of the base installation of R (you do not need to install it). To get the data from a package in your global environment you need to load the package (if it's not already loaded) using the library `function()` and then use the `data()` function. You will see some people use the `require()` function instead of `library()`, but there are [good reasons](http://www.r-bloggers.com/library-vs-require-in-r/) to prefer `library()`.

```{r}
library(MASS)
#Then using the data function we can load one of the datasets in the MASS package, we only need to use the name of the dataframe as an argument (again with no quotes)
data(Boston)
...
You will get the help file that contains information (metadata) about this particular dataset. You can think of it as the codebook for this dataset. You will see that this is a dataset that contains information about crime and other characteristics in various parts of Boston (USA).

You can see all data that are loaded in a session by using the `data()` function with no arguments.
...
As you can see many variables are classed as factors. This is common with survey data. Many of the questions in social surveys measure the anwsers as categorical variables (e.g., these are nominal or ordinal level measures) which are then encoded as factors in R. It is non trivial to know than the default options for read.csv() will encode any character attribute in your file as a factor.
...
```{r}
sum(is.na(BCS0708$educat3)) #This is asking R to sum how many cases are TRUE NA in this variable.
...
If you see more than 5% of the cases being NA, you need to start thinking about the implications of this. Beware of formulaic application if rules of thumb though. There is a whole field of statistics devoted to doing analysis when missing data is a problem. You would cover techniques for dealing with this sort of issues in more advanced courses. For the purpose of this course, we only explain how to do analysis that ignore missing data.
...
In this section we start exploring some statistical summaries for our data. If you have not watched the required video for this week explaining these measures, it is convenient you watch it now -just click [here](https://www.youtube.com/watch?v=5vnxBDau6PU&list=PLX2_QrT3VKR9I8nwfv95Rdqc1_HVU8wMg).
...
There are multiple ways of getting results in R. Particularly for basic and intermediate-level statistical analysis many core functions and packages can give you the answer that you are looking for. For example, there are a variety of packages that allow you to look at summary statistics using functions defined within those packages. You could use `favstats()` function from the [mosaic](http://cran.r-project.org/web/packages/mosaic/index.html) package:
...
If you look at the *mean* for each of the marital status groups, you can see that there is a relationship in this sample. Single people, for example, tend to be younger. Earth shattering discovery! Don't worry, we will look at more interesting examples in subsequent weeks.
...
```{r, eval = FALSE}
#Don't try to execute this, it will tell you there is no object called BritishCrimeSurvey_20072008
...
Next week we will devote the entire session to cover graphics as a way of visualising data. But it is important you understand that although we separate the treatment of this topic, in practice visualising the data is one of the very first things you do (even before you produce nummerical summaries). So, if you are working with a nummerical variable, you may want to produce a histogram to quickly look at the full distribution:
...
When producing frequency distribution tbales, it is particularly useful (if you want to save yourself the work of formatting the tables in a nice way) to use the [sjPlot](http://cran.r-project.org/web/packages/sjPlot/index.html) package. You can use the `sjt.frq()` function for this. This [online tutorial](http://rpubs.com/sjPlot/sjtfrq) explains some of the features of this function in greater detail.
...
Note, of course, that you can also specify a suitable physical location. Once the file is saved as a .RData file you can then load it simply with the `load()` function.
...
If you want to write your data frame into a different format, there are a number of ways for doing so. I won't be expecting you to do so, but you can learn how to do it: [for csv](http://www.screenr.com/lPS8); [for dbf, SPSS, SAS, STATA](http://www.screenr.com/DXd8); and [for Excel](http://www.screenr.com/1Xd8).

The many expressions that you may have been using are better stored within a script. [This video](https://www.youtube.com/watch?v=qqz_D1vzS5M&list=PLqzoL9-eJTNDw71zWePXyHx3_cm_fMP8S&index=10) from the excellent series produced by MarinStats (Mike Marin at University of British Columbia) explain how to work with scripts within RStudio. I highly recommend that you watch it. 
...
Alternatively you may find as helpul reference [the online handouts](http://ateucher.github.io/rcourse_site/index.html) produced by Andy Teucher. They are more parsimonious than this handout and can be used as a cheatsheet for the type of functions we introduced today.
...
[^2]: A "semi-legendary" R programmer and part of the RStudio team. We all want to know as much R as he does.
