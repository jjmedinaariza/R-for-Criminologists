...
author: "Reka Solymosi (based on notes of Juanjo Medina)"
output:
...
In order to follow this handout at home you would need to install both R and R Studio in your local machine 
...
Alternatively, if you are a university student, your university is likely to have these programs already installed in the computers in their labs. 
...
You need one `#` per line, and anything after that is a comment that is not executed by R. You can use spaces after (its not like a hashtag on twitter). 
You do need a `#` for every line you have comments on. 
...
This is actually a table, so before we are able to work with this data, we have to turn it from a table into a dataframe. This is achieved with the `as.data.frame()` function. 
...
```
You can also use things like greater than, lesser than, etc. A list of operators can be found [here](http://www.statmethods.net/management/operators.html).
...
<!--
Let's start by creating a small data frame:
...
You can also filter with the `subset()` function. When applied to vectors, the difference between this function and the ordinary filtering we have covered so far lies in the manner in which missing data is handled. 
...
- [For excel](https://www.r-bloggers.com/read-excel-files-from-r/) 
- [For SPSS](https://www.r-bloggers.com/how-to-open-an-spss-file-into-r/)
- [For JSON](http://stackoverflow.com/questions/2617600/importing-data-from-a-json-file-into-r) 
...
You can see in the Environment window that BCS0708 has 11676 observations (rows) of 35 variables (columns). You can also obtain this information using code. Here you want the  **DIM**ensions of the dataframe (the number of rows and columns) so you use the `dim()` function:
...
<!--
Here we can use the `sapply()` function to ask to the class of the first row of all the columns and then inferring the class from this first row:
...
<!--
`library(sjPlot)`
`sjt.frq(BCS0708$walkdark, variableLabels=c("How safe you feel walking alone at dark?"), no.output=TRUE)$knitr`
-->
...
Or as described in the intro session you can navigate there manually by clicking on Session > Set working directory > choose directory ([see here](https://rawgit.com/maczokni/R-for-Criminologists/master/Intro_day_2.html))
...
+ Assignment operator
+ Debugging
+ Annotations (commenting your code)
+ Functions
+ Arguments
+ Vectors
+ Vector class
...
+ Central tendency
+ Variability
+ Mean, Median, Mode(for more details see [here](https://www.khanacademy.org/math/cc-sixth-grade-math/cc-6th-data-statistics/mean-and-median/v/statistics-intro-mean-median-and-mode))
+ [Standard Deviation]()
+ Range (for more details see [here](http://www.mathsteacher.com.au/year9/ch17_statistics/05_data/analyse.htm#range))
+ Quantiles (for more details see [here](http://www.mathsteacher.com.au/year9/ch17_statistics/06_quartiles/quartiles.htm))
+ Interquartile range (for more details see [here](https://www.youtube.com/watch?v=DGAXeX42eoE))
