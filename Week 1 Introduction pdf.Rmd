...
```{r}
# To create a numeric vector with two elements
my_1st_vector <- c(0.5, 0.6) 
# To create an integer vector
my_2nd_vector <- c(1L, 2L) 
# To create a logical vector
my_3rd_vector <- c(TRUE, FALSE) 
...
```{r}
# To figure out the class of the vector
class(my_1st_vector) 
# To figure the lenght of the vector
length(my_1st_vector) 
# To add a constant to each element of the vector
my_1st_vector + 2 
# To create a new vector that contains the elements of my1stvector plus 1
my_7th_vector <- my_1st_vector + 1 
# To add the two vectors and autoprint the results (note how the sum was done)
my_1st_vector + my_7th_vector 
...
```{r}
#To remove x from your global environment
rm(x) 
```
...
#Then create a factor based on the character vector
the_smiths_f <- as.factor(the_smiths_bis)
#Auto print the factor
the_smiths_f 
```
...
```{r}
# We create a dataframe called mydata.1 with two variables, an integer vector 
# called foo and a logical vector called bar
...
```{r}
#For the 1st, 2nd and 4th elements of the y vector
y[c(1, 2, 4)] 
...
```{r}
#This will return the first row and columns 1 to 2 of the named data frame 
#(anything to the left of the comma refers to rows and to the right to the columns)
...
```{r}
#This will return all the values in the teams column that exist in the 
#Premiership13_14 data frame
...
```{r}
#We are asking for rows where the condition is met, which is why this appears before
#the comma within the brackets, we are not selecting particular columns -and thus 
#don't specify an additional argument after the comma. If you don't specify any 
#condition or list after the comma that means you will include all columns 
#in your dataframe.
...
```{r}
# This creates a vector with 6 values, one of which is NA. NA is the label R 
# uses for unknown values
z <- (c(6, 1:3, NA, 12)) 
z
...
```{r}
#As arguments you need to specify the vector (z) and then the 
#filtering condition (z > 5)
subset(z, z > 5) 
...
download.file("https://raw.githubusercontent.com/jjmedinaariza/LAWS70821/master/BCS0708.csv", 
              "BCS0708.csv", 
              method = "internal")
...
```{r}
#This is asking R to sum how many cases are TRUE NA in this variable. When reading a 
#logical vector as the one we are creating, R will treat the FALSE elements as 0s and
#the TRUE elements as 1s. So basically the sum() function will count the number of 
#TRUE cases returned by the is.na() function.
sum(is.na(BCS0708$educat3)) 
...
```{r}
#Specifying this argument will ensure that only cases with valid values are used in 
#the mathematical operation
mean(BCS0708$age, na.rm = TRUE) 
...
You could use `favstats()` function from the [mosaic](http://cran.r-project.org/web/packages/mosaic/index.html) package  for the five number summary, mean, standard deviation, and number of cases.:

```{r, message=F, warning=F}
library(mosaic)
favstats(~age, data = BCS0708) 

```

The `describe()` function from the [Hmisc](http://cran.r-project.org/web/packages/Hmisc/index.html) package for the n, missing, mean, various percentiles, and the 5 lowest and highest values.:

```{r, message = F, warning=F}
library(Hmisc)
describe(BCS0708$age) 
```

The `stat.desc()` function from the [pastecs](http://cran.r-project.org/web/packages/pastecs/index.html) package, which gives you among other things the standard error, standard deviation, and 95% confidence interval (if you specify norm=TRUE as an argument you will get test for normality of the distribution, but it will only work in data sets size 3 to 5000 -so we cannot use it here). Specifying basic=FALSE as an argument will also excluce some of the descriptives):

```{r, message=F, warning=F}
library(pastecs)
stat.desc(BCS0708$age) 
```

Or the `describe()` function from the [psych](http://cran.r-project.org/web/packages/psych/index.html) package, which gives you a trimmed mean, as well as measures of skew and kurtosis: 
...
Sometimes you want to produce **summary statistics by group**. The `psych` package is good for this. Since we just loaded this package we can continue using its functions

```{r}
#Age descriptives for various categories of marital status
describeBy(BCS0708$age, BCS0708$livharm1) 
...
```{r, eval = FALSE}
#Don't try to execute this, it will tell you there is no object 
#called British_Crime_Survey_2007_2008
...
```{r}
#creates a new object called .Table containing the results we saw before
.Table <- table(BCS0708$walkdark) 
#Percentages for ethnic4 created using the object called .Table we just created 
#(because we did not use useNA, this will compute the valid percent)
round(100*.Table/sum(.Table), 2)  
#removes this object from working memory (we don't need it any longer!)
remove(.Table) 
```
...
Alternatively you may find as helpful reference [the online handouts](http://ateucher.github.io/rcourse_site/index.html) produced by Andy Teucher. They are more parsimonious than this handout and can be used as a cheat-sheet for the type of functions we introduced today. If you want further practice you may enjoy the interactive modules in [Try R](http://tryr.codeschool.com/). And for further documentation, the [Beginner's Guide to R guide](http://www.computerworld.com/article/2497464/business-intelligence/business-intelligence-60-r-resources-to-improve-your-data-skills.html) of Computerworld magazine, is pretty handy and you will find there as well a very comprehensive annotated list of additional learning resources for R.
...
```{r}
#This function provides information about the version of R I used to 
#produce the html file you are reading. All these tutorials have been created directly
#with RStudio using the rmarkdown and knitr packages. A wonderful thing about R is that it
#makes largely redundant the use of other tools for publishing your work or for producing
#presentations. Including this session information at the end of the html file will 
#allow others to reproduce this work if their version of R encounter problems with the 
